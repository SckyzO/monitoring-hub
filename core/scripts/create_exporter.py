...
def get_github_info(repo_name):
    ...
    return {"version": tag_name, "archs": archs, "archive_name": archive_name}
...
@click.option(
    "--repo",
    prompt="GitHub Repository (e.g., prometheus/node_exporter)",
...
def create(name, repo, category, description, show_created_files):
    ...
    # 2. Update Upstream
    manifest["upstream"].update({"repo": repo, "archive_name": archive_name})
