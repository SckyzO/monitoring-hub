...
The tree is served over a local HTTP server reached from the container via
``host.docker.internal``: the RPM split (metadata on Pages, blobs on Releases via
an absolute package URL) only resolves over http(s) — librepo treats an absolute
``file://`` package href as relative to the baseurl. http mirrors production.
"""
...
def _build_signed_tree(tmp_path: Path, tree: Path, base_url: str) -> None:
    """Build pkgs + a signed repo whose blob URLs point at ``base_url``."""
    pkgs = tmp_path / "dist"
    ...
    build_distribution(
        ...
        public_out=tree / "public",
        release_out=tree / "release",
        package_base_url=f"{base_url}/release",
        pages_base_url=f"{base_url}/public",
        key_id=key_id,
        public_key=pub,
...
def _serve(tree: Path) -> http.server.ThreadingHTTPServer:
    """Serve ``tree`` on an ephemeral port; container reaches it via host-gateway."""
    handler = functools.partial(http.server.SimpleHTTPRequestHandler, directory=str(tree))
    httpd = http.server.ThreadingHTTPServer(("0.0.0.0", 0), handler)  # noqa: S104
...
def test_el9_rpm_consumes_signed_repo(tmp_path: Path) -> None:
    ...
    try:
        ...
        _build_signed_tree(tmp_path, tree, f"http://host.docker.internal:{port}")
        script = (
            "set -e; rpm --import /repo/public/RPM-GPG-KEY-monitoring-hub; "
            "printf '[mh]\\nname=mh\\n"
            f"baseurl=http://host.docker.internal:{port}/public/el9/x86_64\\n"
            "enabled=1\\ngpgcheck=0\\nrepo_gpgcheck=1\\n"
...
def test_ubuntu2404_deb_consumes_signed_repo(tmp_path: Path) -> None:
    ...
    try:
        ...
        script = (
            ...
            "echo 'deb [signed-by=/etc/apt/keyrings/mh.asc] "
            f"http://host.docker.internal:{port}/release/apt-noble ./' "
